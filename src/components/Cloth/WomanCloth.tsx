...
const WomanCloth: React.FC = () => {
  const [womanClothDatas, setWomanClothDatas] = useState([]);
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const response = await getProductByCategory("women's clothing");

      setWomanClothDatas(response.data);
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false);
  ...
  return (
    <Container>
      <ContentTitle title="Pakaian Wanita" description={`Pakaian wanita total ${womanClothDatas.length} item`} />
      <div className="grid grid-cols-5 gap-7">
        {womanClothDatas.map((cloths) => {
