...
const ManCloth: React.FC = () => {
  const [manClothDatas, setManClothDatas] = useState([]);
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const response = await getProductByCategory("men's clothing");

      setManClothDatas(response.data);
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false);
